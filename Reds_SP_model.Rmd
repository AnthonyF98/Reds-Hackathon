...
features_matrix <- as.matrix(features1)

set.seed(27)
train_idx <- createDataPartition(target, p = 0.8, list = FALSE)
X_train <- features_matrix[train_idx, ] <- as.numeric(X_train)
y_train <- target[train_idx] <- as.numeric(y_train)
X_test  <- features_matrix[-train_idx, ] <- as.numeric(X_test)
y_test  <- target[-train_idx] <- as.numeric(y_train)
