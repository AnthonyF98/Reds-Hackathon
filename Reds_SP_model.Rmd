...
features1 <- temp11 %>%
  select(-pitcher, -highest_used_fb, -total_batters_faced, -n_appearances, -n, -total_pitches, -`unique(clean_savant$pitcher)`, -debut) %>% 
  na.omit()
...
sp_24 <- temp11 %>% 
  ...
  mutate(PA_last_year = total_batters_faced) %>% 
...
train_data <- sp_24 %>%
  ...
  group_by(pitcher, game_year) %>% 
  mutate(PA_last_year = lag(PA_last_year, 1)) %>%
  filter(game_year != 2021) %>%  # Remove first year due to missing lag
  ungroup()
...
```{r}
X_train <- train_data %>% select(-pitcher, - total_PA) %>% as.matrix()
