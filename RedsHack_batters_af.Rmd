...
# Aggregate data for each batter in 2023
hitter_data_2024 <- hitter_data_clean %>%
  filter(season == 2023) %>%  # Use most recent season
  group_by(batter) %>%  # Group by batter
  summarise(
    ...
    total_games = n_distinct(game_pk),  # Count number of games played
    total_PA = sum(PA, na.rm = TRUE)  # Total plate appearances (Target variable)
  ) %>%
  mutate(season = 2024,  # Set season to 2024 for predictions
         PA_last_year = total_PA) %>%  # Save 2023 PA as last year’s PA
  select(-total_PA)  # Remove total_PA because we are predicting it
...
# Print differences
print(setdiff(train_features, test_features))  # Columns in train but not in test
print(setdiff(test_features, train_features))  # Columns in test but not in train
...
# Convert to DMatrix
dtrain <- xgb.DMatrix(data = X_train, label = train_data$total_PA)
